...
func NewPodBuilder(t testing.TB, manifest model.Manifest) PodBuilder {
	return PodBuilder{
		t:        t,
		manifest: manifest,

		// 5 is enough containers for anybody.
		imageRefs: make([]string, 5),
		cIDs:      make([]string, 5),
	}
}
...
func (b PodBuilder) buildImage(index int) string {
	if b.imageRefs[index] != "" {
		return b.imageRefs[index]
	}
...
func (b PodBuilder) buildContainerID(index int) string {
	...
	return fmt.Sprintf("%s%s%s", k8s.ContainerIDPrefix, testContainer, indexSuffix)
}
...
func (b PodBuilder) Build() *v1.Pod {
	entities, err := ParseYAMLFromManifests(b.manifest)
	if err != nil {
